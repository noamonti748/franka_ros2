#!/usr/bin/env python3
"""Export the RF3 constrained-reach SB3 PPO checkpoint to ONNX.

RF3 uses the same deployment-facing observation/action interface as the
lower-vibration reach/hold checkpoint:

  - joint1..joint7: [qpos/pi, qvel/2.0, prev_qpos/pi, prev_qvel/2.0]
  - prev_action: previous applied/governed 7-joint command divided by pi
  - tcp_pos: (tcp_m - target_m) / 0.75
  - action: absolute 7-joint position target vector in radians

The ONNX graph expects observations already normalized and clipped by the ROS
caller. It exports only the deterministic policy; stateful deployment processing
(low-pass smoothing, rate limiting, velocity governor, optional notch/hold) stays
outside the graph.

Run with the MuJoCo-SB3 venv:

    /home/amd/Robotics/MuJoCo-SB3/.venv/bin/python export_ppo_final_rf3_onnx.py
"""
...
DEFAULT_CHECKPOINT = PACKAGE_DIR / "ppo_final_rf3_balanced.zip"
DEFAULT_OUTPUT = PACKAGE_DIR / "ppo_final_rf3.onnx"


class RF3OnnxPolicyWrapper(nn.Module):
    """Torch module with stable named ONNX I/O for RF3 deployment."""
...
def main() -> None:
    ...
    wrapper = RF3OnnxPolicyWrapper(policy)
    wrapper.eval()
